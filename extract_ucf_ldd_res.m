...
for i=3:length(classlist)
    ...
    mkdir (['ucf','_sum',num2str(flength),'_ldd_res/',classname])
...
%             [feature_conv_normalize_1_1, feature_conv_normalize_1_2] = FeatureMapNormalization(feature_conv_normalize_1);
            [ldd_feature] = UCF_LDD(feature_conv_normalize_1,flength,stride);
            save(['./ucf','_sum',num2str(flength),'_ldd_res/',classname,'/',videoname(1:end-4),'.mat'],'ldd_feature','-v7.3');
        end
    end
end
